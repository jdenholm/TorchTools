...
def test_in_chans_arg_types():
    ...
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(64, 64))
    _ = VAE2d(in_chans=2, out_chans=3, input_dims=(64, 64))

    # Should break with non-int
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1.0, out_chans=3, input_dims=(64, 64))
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1j, out_chans=3, input_dims=(64, 64))
...
def test_in_chans_arg_values():
    ...
    with pytest.raises(ValueError):
        _ = VAE2d(in_chans=0, out_chans=3, input_dims=(64, 64))
...
def test_out_chans_arg_types():
    ...
    # Should work with positive int
    _ = VAE2d(in_chans=1, out_chans=1, input_dims=(64, 64))
    _ = VAE2d(in_chans=2, out_chans=1, input_dims=(64, 64))

    # Should break with non-int
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=1.0, input_dims=(64, 64))
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=1j, input_dims=(64, 64))
...
def test_out_chans_arg_values():
    ...
    with pytest.raises(ValueError):
        _ = VAE2d(in_chans=1, out_chans=0, input_dims=(64, 64))
    with pytest.raises(ValueError):
        _ = VAE2d(in_chans=1, out_chans=-1, input_dims=(64, 64))
    with pytest.raises(ValueError):
        _ = VAE2d(in_chans=1, out_chans=-2, input_dims=(64, 64))
...
def test_input_dims_arg_type():
    ...
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims={64, 12})
...
def test_input_dims_arg_length():
    ...
    with pytest.raises(RuntimeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(64,))

    with pytest.raises(RuntimeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(64, 64, 64))
...
def test_input_dims_arg_elements_types():
    ...
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(64.0, 64))

    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(64, 64.0))

    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(64j, 32j))


def test_input_dims_arg_element_values():
    """Test the values of the contents of ``input_dims``."""
    # Should work with tuple of ints
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32))
...
def test_start_features_argument_values():
    ...
    # Should work with positive ints
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), start_features=1)
...
def test_num_layers_argument_types():
    ...
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), num_layers=2)

    # Should break with non-ints
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), num_layers=2.0)
...
def test_num_layers_argument_values():
    ...
    with pytest.raises(ValueError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), num_layers=1)

    with pytest.raises(ValueError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), num_layers=0)


def test_down_pool_argument_types():
    """Test the types accepted by the ``down_pool`` argument."""
    # Should work with str
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), down_pool="max")

    # Should break with non-str
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), down_pool=123)
...
def test_down_pool_argument_values():
    ...
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), down_pool="max")
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), down_pool="avg")

    # Should break with any other string
    with pytest.raises(KeyError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), down_pool="Galadriel")


def test_the_bilinear_argument_types():
    """test the types accepted by the ``bilinear`` argument type."""
    # Should work with bool
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), bilinear=True)
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), bilinear=False)

    # Should work with non-bool
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), bilinear=123)


def test_lr_slope_argument_types():
    """Test the types accepted by the ``lr_slope`` argument."""
    # Should work with floats
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), lr_slope=0.0)
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), lr_slope=0.1)

    # Should break with non-float
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), lr_slope=0)

    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), lr_slope=1.0j)


def test_kernel_size_argument_type():
    """Test the types accepted by the ``kernel_size`` argument."""
    # Should work with odd, positive ints
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), kernel_size=1)
    _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), kernel_size=3)

    # Should break with non-int
    with pytest.raises(TypeError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), kernel_size=1j)
...
def test_kernel_size_argument_values():
    ...
    with pytest.raises(ValueError):
        _ = VAE2d(in_chans=1, out_chans=3, input_dims=(32, 32), kernel_size=0)
